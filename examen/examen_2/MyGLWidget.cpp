...
void MyGLWidget::iniEscena ()
{
  ...
  //radiEsc = sqrt(5);  
  radiEsc = distance(glm::vec3(0,0,0), glm::vec3(2,1,2));  
}
...
void MyGLWidget::iniCamera ()
{
  angleY = angleX = 0.0;
  ra = 1.0;
  fov = 2 * (float(M_PI/3.0)/2);
  zn = 0.1;
  zf = 3*radiEsc;
    isFocusSCA = 0;
  perspectiva = true;
emit activaCamera(true);
  projectTransform ();
  viewTransform ();
}
...
void MyGLWidget::resizeGL (int w, int h) 
{
  ...
  if(ra < 1) fov = 2 * atan(tan(float(M_PI/3.0)/2)/ra);
  else fov = 2 * (float(M_PI/3.0)/2);
  projectTransform();
}
...
void MyGLWidget::modelTransformPatricio ()
{
  ...
  std::cout << "escalaPat: " << escalaPat << std::endl;
  TG = glm::scale(TG, glm::vec3(0.25*escalaPat, 0.25*escalaPat, 0.25*escalaPat));
  TG = glm::rotate(TG, girR, glm::vec3(0, 1, 0));
...
void MyGLWidget::modelTransformCow ()
{
  ...
  std::cout << "escalacow: " << escalaCow << std::endl;
  TG = glm::scale(TG, glm::vec3(0.5*escalaCow, 0.5*escalaCow, 0.5*escalaCow));
  TG = glm::rotate(TG, float((270*M_PI))/float(180.0), glm::vec3(0, 1, 0));
    TG = glm::rotate(TG, girR, glm::vec3(0, 1, 0));
  TG = glm::rotate(TG, float((270*M_PI))/float(180.0), glm::vec3(1, 0, 0));
...
void MyGLWidget::projectTransform ()
{
  ...
  glUniform1i(focusSCALoc, isFocusSCA);
...
*/
    ...
    glm::mat4 View = glm::lookAt (glm::vec3(-1 ,1,-1),  glm::vec3(1, -0.625, 0), glm::vec3(0 ,1,0));  
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) 
{
  ...
  switch (event->key()) {
    ...
    case Qt::Key_R: {
        girR += float(M_PI)/float(6.0);
        std::cout <<  "girR: " << girR << std::endl;
        break;
    }
    case Qt::Key_L: {
        if(isFocusSCA == 0) {
            isFocusSCA = 1;
            emit activaEscena(true);
        }
        else {
            isFocusSCA = 0;
            emit activaCamera(true);
...
void MyGLWidget::focusCamera(bool b){
    std::cout << "ha activado foco camara" << std::endl;
    makeCurrent();
    isFocusSCA = 0;
    projectTransform();
    update();
...
void MyGLWidget::focusEscena(bool b){
    std::cout << "ha activado foco escena" << std::endl;
    makeCurrent();
    isFocusSCA = 1;
    projectTransform();
    update();
...
void MyGLWidget::calculaCapsaCow (Model &p, float &escala, glm::vec3 &centreBase)
{
  // Càlcul capsa contenidora i valors transformacions inicials
  float minx, miny, minz, maxx, maxy, maxz;
  minx = maxx = p.vertices()[0];
  ...
  //centreBase[0] = (minx+maxx)/2.0; centreBase[1] = miny; centreBase[2] = (minz+maxz)/2.0;
  centreBase[0] = (minx+maxx)/2.0; centreBase[1] = (miny+maxy)/2.0; centreBase[2] = minz;
  std::cout << "vaca: " << escala << std::endl;
}
...
void MyGLWidget::creaBuffersCow ()
{
  // Carreguem el model de l'OBJ - Atenció! Abans de crear els buffers!
  cow.load("./models/cow.obj");

  // Calculem la capsa contenidora del model
  calculaCapsaCow (cow, escalaCow, centreBaseCow);
...
void MyGLWidget::carregaShaders()
{
  ...
  focusSCALoc = glGetUniformLocation (program->programId(), "focusSCA");
}
