...
void MyGLWidget::iniEscena ()
{
  ...
  creaBuffersTerraIParet();
    posX = 0;
    focusPatrPet = 0;
  centreEsc = glm::vec3 (0, 1.25, 0);
  //radiEsc = sqrt(5);  
  radiEsc = distance(centreEsc, glm::vec3(2,2.5,2));
}
...
void MyGLWidget::iniCamera ()
{
  ...
  //fov = float(M_PI/3.0);
  //std::cout << "fovhardcoded : " << fov << std::endl;
  zn = radiEsc;
  zf = 3*radiEsc;
...
void MyGLWidget::resizeGL (int w, int h) 
{
  ...
  if(camaraPatricio){
        if(ra < 1) fov = 2 * atan(tan(float(M_PI/4.0))/ra);
        else fov = 2 * M_PI/4.0;
    } else {
        if(ra < 1) fov = 2 * atan(tan(asin(float(radiEsc/(2*radiEsc))))/ra);
        else fov = 2 * asin(float(radiEsc/(2*radiEsc)));
    }
  projectTransform();
}
...
void MyGLWidget::modelTransformPatricio ()
{
  ...
  TG = glm::translate(TG, glm::vec3(-2 + posX, -1, 0));
  TG = glm::scale(TG, glm::vec3(1*escalaPat, 1*escalaPat, 1*escalaPat));
  TG = glm::rotate(TG, float(90*M_PI/180), glm::vec3(0, 1, 0));
  TG = glm::translate(TG, -centreBasePat);
  posFocusPatrPet = glm::vec3(-2 + posX, 0, 0);
  
  glUniform1i(focusPatrPetLoc, focusPatrPet);
  glUniform3fv(posFocusPatrPetLoc, 1, &posFocusPatrPet[0]);
...
void MyGLWidget::modelTransformPatricio2 ()
{
  ...
  TG = glm::scale(TG, glm::vec3(2.5*escalaPat, 2.5*escalaPat, 2.5*escalaPat));
  TG = glm::rotate(TG, float(-90*M_PI/180), glm::vec3(0, 1, 0));
  TG = glm::translate(TG, -centreBasePat);
...
void MyGLWidget::viewTransform ()
{
  ...
  if(camaraPatricio){
      View = glm::lookAt (glm::vec3(-2 + posX ,1.5,0),  glm::vec3(2 ,0.25,2), glm::vec3(0 ,1,0));  
    } else {
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) 
{
  ...
  switch (event->key()) {
    ...
    case Qt::Key_A: {
        std::cout << "patrisio mueve x+" << std::endl;
        posX += 0.2;
        projectTransform();
        viewTransform();
        break;
    ...
    case Qt::Key_F: {
        std::cout << "foco encima patrisio" << std::endl;
        if(focusPatrPet == 1) focusPatrPet= 0;
        else focusPatrPet = 1;
        modelTransformPatricio();
        break;
    }
    case Qt::Key_C: {
        ...
        if(camaraPatricio) emit enviaCamara(true);
        else emit enviaCamara(false);
        projectTransform();
        viewTransform();
        break;
    }
...
void MyGLWidget::recibeCamara(bool camara){
    makeCurrent();
    camaraPatricio = camara;
    projectTransform();
    viewTransform();
    update();
}
...
void MyGLWidget::carregaShaders()
{
  ...
  focusPatrPetLoc = glGetUniformLocation (program->programId(), "focusPatrPet");
    posFocusPatrPetLoc = glGetUniformLocation (program->programId(), "posFocusPatrPet");
